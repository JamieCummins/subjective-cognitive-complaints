...
con <-
  dbConnect(drv = RSQLite::SQLite(),
            ...
            dbname = paste0('raw/questionnaire_', format(Sys.Date(), "%d%m%Y"), '.sqlite'))
...
write_csv(full_raw_data_df, paste0("raw/questionnaire_", format(Sys.Date(), "%d%m%Y"), ".csv"))
...
# CHANGE LOCATION AND NAME TO WHERE THE .CSV FILE IS SAVED
questionnaire_raw_df <- read_csv("raw/questionnaire_06052022.csv") 
...
myCsv <- getURL("ftp://ftp.drjamiecumminscom.webhosting.be/md/vragenlijst/data/data.sqlite", ssl.verifypeer = FALSE)
myData <- read.csv(textConnection(myCsv))
myData

"ftp://ftp.drjamiecumminscom.webhosting.be/md/vragenlijst/data/data.sqlite"
