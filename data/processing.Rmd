...
con <-
  dbConnect(drv = RSQLite::SQLite(),
            ...
            dbname = paste0('raw/training/smart_data_', format(Sys.Date(), "%d%m%Y"), '.sqlite'))
...
write_csv(full_raw_data_df, paste0("raw/smart_", format(Sys.Date(), "%d%m%Y"), ".csv"))
...
smart_raw_df <- read_csv(paste0("raw/smart_", format(Sys.Date(), "%d%m%Y"), ".csv"))

view_df <- smart_raw_df |>
  group_by(unique_id) |>
  fill(email, .direction = "downup") |>
  ungroup() |>
  count(email, unique_id, str_sub(timestamp, 1, 10))
